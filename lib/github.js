...
async function downloadFromGithub(repoUrl, options) {
  ...
  if (packageJsonItem) {
    ...
    // const { nodeInitIgnore = ["api", "user", "/a/**"] } = packageJson;
    const { nodeInitIgnore = ["api", "user", "a/**"] } = packageJson;
    console.log("Apply ignore files from package.json:", nodeInitIgnore);
    if (nodeInitIgnore) {
      // regexp: ^((?!(api|user)).+)$
      const regexpstr = globToRegexp(nodeInitIgnore, {globstar: true, ignore: false});//`^((?!(${nodeInitIgnore.join("|")})).)*$$`;
      console.log("regexpstr:", regexpstr);
      const regexp = new RegExp(regexpstr);
      toBeDownloaded = toBeDownloaded.filter((e) => regexp.test(e.path));
    }
  }
